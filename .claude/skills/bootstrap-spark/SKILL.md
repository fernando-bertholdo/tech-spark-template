...
description: Kickoff guiado para projetos criados a partir de tech-spark-template. Use no primeiro contato com o repo recém-clonado para popular Projeto.md, README.md e .claude/CLAUDE.md a partir da ideia (vaga, brief existente, ou já-pronto). Substitui o caminho manual do KICKOFF_GUIDE.md.
---

# Bootstrap Spark — Kickoff Guiado

Inicialização guiada de projetos `tech-spark-template`. Substitui placeholders em `Projeto.md`, `README.md` e `.claude/CLAUDE.md` a partir de 3 modos de captura de contexto: greenfield (ideia vaga), brief (material existente), ready (decisões prontas).
...
- Quando arquivos `Projeto.md`, `README.md`, `.claude/CLAUDE.md` ainda contêm placeholders `{{...}}`
...
```bash
grep -lE '\{\{[A-Z_]+\}\}' Projeto.md README.md .claude/CLAUDE.md 2>/dev/null
```
...
|---|---|---|
| **Fresh** | Todos os 3 | Seguir modo escolhido normalmente |
| **Parcial** | 1 ou 2 dos 3 | Perguntar: "Detectei bootstrap parcial em `<arquivos>`. Retomar (`--resume`) ou recomeçar?" — se recomeçar, restaurar manualmente |
| **Bootstrappado** | Nenhum | **Falhar**: `"Projeto já bootstrappado (nenhum placeholder restante em Projeto.md/README.md/.claude/CLAUDE.md). Para refazer, restaure o template e tente novamente."` |
...
|---|---|---|
| `{{PROJECT_NAME}}` | `README.md`, `Projeto.md`, `.claude/CLAUDE.md` | Resposta do usuário (greenfield Q "nome do projeto") ou `PROJECT_NAME:` no bloco do modo ready ou flag `--name`. Default: `basename "$PWD"` |
...
| `{{AUTHOR_NAME}}` | `README.md`, `.claude/CLAUDE.md` | `git config user.name` (auto). Se vazio, perguntar |
| `{{AUTHOR_EMAIL}}` | `README.md` | `git config user.email` (auto). Se vazio, perguntar |
| `{{DATE}}` | `Projeto.md`, `.claude/CLAUDE.md` | Data ISO 8601 atual: `date +%Y-%m-%d` |
| `{{COMMIT_SCOPES}}` | `.claude/CLAUDE.md` | Inferido do Stack — ver tabela abaixo |
...
Ao final, os 3 arquivos abaixo ficam **sem placeholders**:
...
7. Substituir placeholders nos 3 arquivos (sed/edit, em ordem):
   - Projeto.md
   - README.md
   - .claude/CLAUDE.md
8. Verificação final:
   - `grep -lE '\{\{[A-Z_]+\}\}' Projeto.md README.md .claude/CLAUDE.md`
...
- Arquivos modificados: Projeto.md, README.md, .claude/CLAUDE.md
- Próximo passo: revise os arquivos e commite com:
    git add Projeto.md README.md .claude/CLAUDE.md
...
- [.claude/CLAUDE.md](../../CLAUDE.md) — destino terciário (regras operacionais)
